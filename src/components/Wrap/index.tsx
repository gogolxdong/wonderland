...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const memoAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.slwh;
    });
    ...
    const getBalance = () => (isWrap ? `${trimmedMemoBalance} sLWH` : `${trimmedWmemoBalance} wMEMO`);
    ...
    return (
        <Modal id="hades" open={open} onClose={onClose} hideBackdrop>
            <Paper className="lwh-card lwh-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "sLWH" : "wMEMO"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wMEMO" : "sLWH"}`}
                        </p>
                    </div>
                </div>
